...
export default async function setup() {
  ...
  for (const r of stale.rows as { datname: string }[]) await adminQuery(`DROP DATABASE IF EXISTS "${r.datname}"`);
  return async () => {
    ...
    for (const r of left.rows as { datname: string }[]) {
      await adminQuery(`SELECT pg_terminate_backend(pid) FROM pg_stat_activity WHERE datname = $1 AND pid <> pg_backend_pid()`, [r.datname]);
      await adminQuery(`DROP DATABASE IF EXISTS "${r.datname}"`);
    }
  };
}
