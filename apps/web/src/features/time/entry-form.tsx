...
export const EntryForm = ({ open, onOpenChange, entry, revise, defaultTaskId, defaultDate }: EntryFormProps) => {
  ...
  // `entry` is the row as the drawer opened; after a conflict only changed fields are sent (T162).
  const edit = useEditBase(entry, { open: open && !!entry && !revise });
  ...
  const submit = async () => {
    ...
    try {
      const r = revise && entry
        ...
        : entry
          ? await update.run({ params: { workspaceId: workspace.id, entryId: entry.id }, body: changedOnly(fields, edit.start ?? entry) }, { ifMatch: edit.version })
      ...
      if (isApiError(err)) {
        ...
        if (!err.fieldErrors.length) fe.form = err.message;
        setErrors(fe);
      } else setErrors({ form: 'The entry could not be saved.' });
    }
  };

  /** The fields that differ from the entry as it was opened (the time fields travel together). */
  const changedOnly = <F extends Record<string, unknown>>(fields: F, x: NonNullable<typeof entry>): Partial<F> => {
    const m = Math.round((x.durationSeconds ?? 0) / 60);
    const before = {
      taskId: x.task.id,
      startedAt: x.startedAt ? fromLocalInput(toLocalInput(x.startedAt, tz), tz) : null,
      endedAt: x.endedAt ? fromLocalInput(toLocalInput(x.endedAt, tz), tz) : null,
      durationMinutes: x.startedAt ? null : m,
      workDate: x.startedAt ? undefined : x.workDate,
      note: x.note?.trim() || null,
      ...(showBillable ? { billable: !!x.billable } : {}),
    };
    const changed = changedFields(before as unknown as F, fields);
    const time = ['startedAt', 'endedAt', 'durationMinutes', 'workDate'];
    if (changed.some((k) => time.includes(String(k)))) changed.push(...(time as (keyof F)[]));
    return pickChanged(fields, changed);
  };
